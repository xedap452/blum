...
def check_ip(proxy):
    try:
        ...
        if ip_info['status'] == 'success':
            print(f"{Fore.GREEN}Proxy IP: {ip_info['query']} | Country: {ip_info['country']}")
            return ip_info['query']
        else:
            print(f"{Fore.RED}Không thể kiểm tra IP proxy")
            return None
    except Exception as e:
        print(f"{Fore.RED}Lỗi khi kiểm tra IP proxy: {e}")
        return None
...
def get_user_info(token, proxy):
    ...
    response = requests.get('https://gateway.blum.codes/v1/user/me', headers=headers, proxies={"http": proxy, "https": proxy})
    if response.status_code == 200:
        ...
        if hasil['message'] == 'Token is invalid':
            ...
            if new_token:
                ...
                return get_user_info(new_token, proxy)  
            else:
...
def get_balance(token, proxy):
    ...
    response = requests.get('https://game-domain.blum.codes/api/v1/user/balance', headers=headers, proxies={"http": proxy, "https": proxy})
    return response.json()
...
def play_game(token, proxy):
    ...
    response = requests.post('https://game-domain.blum.codes/api/v1/game/play', headers=headers, proxies={"http": proxy, "https": proxy})
    return response.json()


def claim_game(token, game_id, points, proxy):
...
def claim_balance(token, proxy):
    ...
    response = requests.post('https://game-domain.blum.codes/api/v1/farming/claim', headers=headers, proxies={"http": proxy, "https": proxy})
    return response.json()
...
def start_farming(token, proxy):
    ...
    response = requests.post('https://game-domain.blum.codes/api/v1/farming/start', headers=headers, proxies={"http": proxy, "https": proxy})
    return response.json()

def refresh_token(old_refresh_token, proxy):
...
def check_balance_friend(token, proxy):
    headers = {
        'Authorization': f'Bearer {token}',
        'accept': 'application/json, text/plain, */*',
        'accept-language': 'en-US,en;q=0.9',
    ...
    response = requests.get('https://gateway.blum.codes/v1/friends/balance', headers=headers, proxies={"http": proxy, "https": proxy})
    balance_info = response.json()
    return balance_info

def claim_balance_friend(token, proxy):
    headers = {
        'Authorization': f'Bearer {token}',
        'accept': 'application/json, text/plain, */*',
        'accept-language': 'en-US,en;q=0.9',
    ...
    response = requests.post('https://gateway.blum.codes/v1/friends/claim', headers=headers, proxies={"http": proxy, "https": proxy})
    return response.json()

def check_daily_reward(token, proxy):
    headers = {
        'Authorization': f'Bearer {token}',
        'accept': 'application/json, text/plain, */*',
        'accept-language': 'en-US,en;q=0.9',
    ...
    response = requests.post('https://game-domain.blum.codes/api/v1/daily-reward?offset=-420', headers=headers, proxies={"http": proxy, "https": proxy})
    try:
        return response.json()
    except ValueError:
        print(f"\r{Fore.RED+Style.BRIGHT}Không thể nhận phần thưởng hàng ngày")
        return None
...
while True:
    for query_id, proxy in zip(query_ids, proxies):
        current_ip = check_ip(proxy)
        if not current_ip:
            print(f"{Fore.RED+Style.BRIGHT}Proxy không hợp lệ hoặc không thể kết nối. Thử lại với proxy khác...")
            continue 
        else:
            print(f"{Fore.GREEN+Style.BRIGHT}Proxy hiện tại: {current_ip}")
        token = get_new_token(query_id, proxy)
        ...
        balance_info = get_balance(token, proxy)
        ...
        if farming_info:
            ...
            print(f"Thời gian nhận farming: {hours_remaining} giờ {minutes_remaining} phút | Số token: {farming_info['balance']}")
        else:
            print(f"{Fore.RED+Style.BRIGHT}Thông tin về farming không có sẵn")
            hours_remaining = 0
            minutes_remaining = 0
            start_response = start_farming(token, proxy)
            if start_response:
                print(f"\r{Fore.GREEN+Style.BRIGHT}Farming đã bắt đầu.", flush=True)
            else:
                print(f"\r{Fore.RED+Style.BRIGHT}Không thể bắt đầu farming", flush=True)
        print(f"\r{Fore.YELLOW+Style.BRIGHT}Đang kiểm tra phần thưởng hàng ngày...", end="", flush=True)
        daily_reward_response = check_daily_reward(token, proxy)
        if daily_reward_response is None:
            ...
            if daily_reward_response['message'] == 'OK':
                print(f"\r{Fore.GREEN+Style.BRIGHT}Phần thưởng hàng ngày đã được nhận thành công!", flush=True)
            elif daily_reward_response['message'] == 'same day':
                print(f"\r{Fore.GREEN+Style.BRIGHT}Phần thưởng hàng ngày đã được nhận hôm nay", flush=True)
        if hours_remaining < 0:
            print(f"\r{Fore.GREEN+Style.BRIGHT}Đang nhận số dư...", end="", flush=True)
            claim_response = claim_balance(token, proxy)
            if claim_response:
        ...
        friend_balance = check_balance_friend(token, proxy)
        if friend_balance:
            if friend_balance['canClaim']:
                ...
                claim_friend_balance = claim_balance_friend(token, proxy)
        ...
        while balance_info['playPasses'] > 0:
            ...
            game_response = play_game(token, proxy)
            print(f"\r{Fore.CYAN+Style.BRIGHT}Đang kiểm tra game...", end="", flush=True)
            time.sleep(1)
            claim_response = claim_game(token, game_response['gameId'], 2000, proxy)
